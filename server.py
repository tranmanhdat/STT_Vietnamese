...
def login():
    ...
    if request.method=="POST":
        ...
        isExist=checkExistInDb(username,password)
        if isExist==True:
            conn=sqlite3.connect("database.db")
            conn.row_factory=sqlite3.Row
            cur=conn.cursor()
            cur.execute("SELECT * FROM user where username='{0}' and password='{1}'".format(username,password))
            rows=cur.fetchall() 
            for item in rows:
                id=item['id']
                name=item['name']
...
def signup():
    if request.method=="POST":
        ...
        isExist=checkExistInDb(email,password)
        if isExist==False:
            flag=True
            with sqlite3.connect("database.db") as conn:
                try:                    
                    cur=conn.cursor()
                    cur.execute("INSERT INTO user (name,username,password,author)  values ('{0}','{1}','{2}',{3})".format(name,email,password,author))
                    conn.commit()                               
                except:
                    flag=False
                    conn.rollback()
...
def createExams():
    if request.method=="POST":
        ...
        with sqlite3.connect("database.db") as conn:
            try:                    
                cur=conn.cursor()
                cur.execute("INSERT INTO collection (id_user,tittle,topic,content,type,level)  values ({0},'{1}','{2}','{3}',{4},'{5}')".format(id_user,inf['tittle'],inf['topic'],content,inf['typeTest'],inf['level_Test']))
                conn.commit()                               
            except:
                flag=False
                conn.rollback()
...
def createTests():
    if request.method=="POST":
        ...
        with sqlite3.connect("database.db") as conn:
            try:                    
                cur=conn.cursor()
                cur.execute("INSERT INTO test_collection (id_collect,time_Todo,time_submit)  values ({0},{1},'{2}')".format(inf['id_collection'],inf['time_ToDo'],time))
                conn.commit()                               
            except:
                flag=False
                conn.rollback()
        ...
        conn.row_factory=sqlite3.Row
        cur=conn.cursor()
        cur.execute("SELECT *  FROM collection where type=2 and id_user={0}".format(id_user))
        rows=cur.fetchall() 
        return render_template("CreateTests.html",rows=rows,name=session['name'])
...
def selectTests():
    ...
    conn.row_factory=sqlite3.Row
    cur=conn.cursor()
    cur.execute("SELECT ts.*,col.*  FROM test_collection ts,collection col where ts.id_collect=col.id")
    rows=cur.fetchall() 
...
#check login
def checkExistInDb(username,password):
    conn=sqlite3.connect("database.db")
    conn.row_factory=sqlite3.Row
    cur=conn.cursor()
    cur.execute("SELECT * FROM user where username='{0}' and password='{1}'".format(username,password))
